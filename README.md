...
2. **GovConnect Engine** (located in the root `/core` and `/modules` folders): The core Decision Support Engine, featuring the multi-criteria officer scoring model, Explainable AI confidence generation, and the real-time simulation dashboard.
...
- [GovConnect Decision Intelligence Simulator](file:///c:/HACAKTHONS/SNIST/code%20files/ai-catalyst/index.html)
...
### 2. GovConnect Simulator (Vite Server)
```bash
npm install
npm run dev
```
